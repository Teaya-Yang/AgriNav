...
public:
  ...
  static Rotation FromEulerYPR(const Real &y, const Real &p, const Real &r)
  { // NB! rotation: 3-2-1 yaw,pitch,roll
    Rotation rot;
    rot[0] = tcos(half * y) * tcos(half * p) * tcos(half * r) + tsin(half * y) * tsin(half * p) * tsin(half * r);
    rot[1] = tcos(half * y) * tcos(half * p) * tsin(half * r) - tsin(half * y) * tsin(half * p) * tcos(half * r);
    rot[2] = tcos(half * y) * tsin(half * p) * tcos(half * r) + tsin(half * y) * tcos(half * p) * tsin(half * r);
    rot[3] = tsin(half * y) * tcos(half * p) * tcos(half * r) - tcos(half * y) * tsin(half * p) * tsin(half * r);
    return rot;
  }
  ...
  static Rotation FromRotationMatrix(const Matrix<Real, 3, 3> a)
  {
