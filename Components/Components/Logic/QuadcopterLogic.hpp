...
namespace Onboard
{
    ...
    public:
        ...
        void SetUWBMeasurement(float const range, uint8_t const responder_id, bool const failure)
        {
            // TODO, nicer logic here
            _timeSinceLastUwb.Reset();
            _uwbRangeMeas.isNew = true;
            _uwbRangeMeas.targetId = responder_id;
            _uwbRangeMeas.range = range;
            _uwbRangeMeas.failure = failure;
        }

        inline Vec3f GetAccelerometer(void) const { return _imuAccelerometer.rawMeas; }

        inline Vec3f GetRateGyroBiasCorrected(void) const { return (_imuRateGyro.rawMeas - _gyroCalibrationBias); }

        inline float GetIMUTemperature() const { return _imuTemperature.rawMeas; }

        inline float GetBatteryVoltage() const { return _battMeas.voltageRaw; }

        inline float GetBatteryCurrent() const { return _battMeas.currentRaw; }

        int GetCycleCounter() const { return _cycleCounter; }

        int GetUWBMeasurementCounter() const { return _uwbRangeMeas.count; }
        ...
        void SetGyroCalibration(bool flag)
        {
            if (_gyroCalibrationEnabled && !flag)
            {
                ...
                {
                    _gyroCalibrationBias = _gyroCalibrationAccumulated / float(_gyroCalibrationNumSamples);
                }
                else
                {
        ...
        bool GetGyroCalibration() const { return _gyroCalibrationEnabled; }
        ...
        enum FlightState
        {
            ...
            FS_EXTERNAL_ACCELERATION_CONTROL_WITH_YAW,
            FS_EXTERNAL_ACCELERATION_CONTROL,
            FS_EXTERNAL_RATES_CONTROL,
            FS_EXTERNAL_SPEEDS_CONTROL,
            FS_EXTERNAL_TILTROTOR_CONTROL,
        };

        FlightState GetFlightState(void) const { return _state; }

        void GetEstimate(Vec3f &pos, Vec3f &vel, Rotationf &att, Vec3f &angVel) const
        {
            pos = _kf.GetPosition();
            vel = _kf.GetVelocity();
            att = _kf.GetAttitude();
        ...
        bool ShouldStartNewUWBConversation(void) const { return _shouldStartNewUWBConversation; }
        ...
        uint8_t GetCurrentUWBRangingTarget(void) const { return _uwbRangeMeas.targetId; }

        /* PX4-ESP telemetry packet construction */
        void GetESPTelemetryDataPacket(TelemetryPacket::esp_data_packet_t &esp_data_packet);

        void GetTelemetryDataPackets(TelemetryPacket::data_packet_t &dataPacket1, TelemetryPacket::data_packet_t &dataPacket2,
        ...
        float GetPropellerCorrectionFactor(unsigned i) const { return _propellerCalibration.activeFactors[i]; }
        ...
        float GetDebugFloats(unsigned i) const { return _debug[i]; }

        void UpdateRangingDiagnostics(unsigned id, float range, bool failure);

        void PrintStatus() const;

        void ClearRangingTargets() { _numRangingTargets = 0; }
        ...
        int GetLogicRunNumber() const { return _cycleCounter; }

        int GetFirstPanicReason() const { return _firstPanicReason; }

        /* PX4-ESP uorb variables */
        // ESC telemetry
        struct
        {
            ...
            float voltage[4];
            float current[4];
            // float consumption[4];  // not in PX4
            int32_t rpm[4];
        } esc_telemetry;
        // distance sensor
        struct
        ...
        /* PX4-ESP other variables */
        bool status_bits[8];
        float esp_debug_vals[TelemetryPacket::ESPPacketSize::DEBUG];
        bool dshot_direct;
        uint16_t dshot_values[4];
        int bidirectional;

        /* Other variables */
        Rotationf _R; // Rotation matrix corresponding to IMU's rotation

        /* Tiltrotor variables */
        struct
        {
            float angle_command[2];
            float angle_feedback[2];
            float temperature_feedback[2];
            float voltage_feedback[2];
        } lobot_servo;
    ...
    protected:
        ...
        virtual void RunControllerExternalAccelerationWithYaw();
        virtual void RunControllerExternalAcceleration();
        virtual void RunControllerExternalTiltrotor();
        virtual void RunControllerExternalRatesControl();
        virtual void RunControllerExternalSpeedsControl();
        virtual void RunControllerFullyAutonomous();

    private:
        int GetRangingTargetPosition(const uint8_t id, Vec3f &posOut);

        bool AreInSafetyCriticalFlightState()
        { // Use this for safety checks, for example
            switch (_state)
            {
        ...
        TiltrotorController _tiltrotorController;
        ...
        {
            MonitorSimplePeriod(BaseTimer *const timeIn, float samplePeriod, float cuttoffFrq, float expectedDtIn) : timeSinceLast(timeIn)
            {
        ...
        {
            MonitorUWBEstimatorResets(BaseTimer *const timeIn) : timeSinceLast(timeIn)
            {
                // nothing.
            }
