...
class QuadcopterMixer {
  public:
    ...
    QuadcopterMixer() : _dx(0), _dy(0), _kt(0), _kf(0) {
    ...
    void SetParameters(float armLength, float armAngle, float propellerThrustFromSpeedSqr, float propellerTorqueFromThrust, int prop0SpinDir,
                       float maxThrustPerPropeller, float minThrustPerPropeller, float maxCmdTotalThrust = -1.0f) {
        _dx = armLength * cos(armAngle);
        _dy = armLength * sin(armAngle);
    ...
    float GetPropellerCorrectionFactors(unsigned const i) const { return _corrFac[i]; }
    ...
    void GetMotorForces(const float totF, const Vec3f t, float outF[4]) {
        ...
        outF[0] = (-t.x / _dy - t.y / _dx - t.z / _kt + desF) / 4.0f;
        outF[1] = (-t.x / _dy + t.y / _dx + t.z / _kt + desF) / 4.0f;
        outF[2] = (+t.x / _dy + t.y / _dx - t.z / _kt + desF) / 4.0f;
        outF[3] = (+t.x / _dy - t.y / _dx + t.z / _kt + desF) / 4.0f;
    ...
    void PropellerSpeedsFromThrust(float const thrusts[4], float outSpeeds[4]) const {
        for (int i = 0; i < 4; i++) {
    ...
    float GetUncorrectedForce(float const inSpeed) const { return _kf * inSpeed * inSpeed; }

  private:
    float _dx;  // distance from COM to propeller center
    float _dy;
