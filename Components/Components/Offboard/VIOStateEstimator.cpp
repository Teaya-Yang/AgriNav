...
VIOStateEstimator::VIOStateEstimator(BaseTimer* const masterTimer,
                                         unsigned const id,
  ...
  // _measNoiseStdDevVel = 0.02;  //[m/s]
  // _measNoiseStdDevAngVel = 15 * M_PI / 180;  //[rad/s] Guestimate from data
  _measNoiseStdDevPos = 0.0;  //[m]
  _measNoiseStdDevAtt = 0.0;  //[rad]
...
void VIOStateEstimator::ResetVariance() {
...
VIOStateEstimator::VIOEstimatedState VIOStateEstimator::GetPrediction(
  ...
  if (std::isnan(est.pos.x) || std::isnan(est.pos.y) || std::isnan(est.pos.z) ||
      std::isnan(est.vel.x) || std::isnan(est.vel.y) || std::isnan(est.vel.z) ||
      std::isnan(est.att[0]) || std::isnan(est.att[1]) || std::isnan(est.att[2]) || std::isnan(est.att[3]) ||
      std::isnan(est.angVel.x) || std::isnan(est.angVel.y) || std::isnan(est.angVel.z)) {
    
    std::cerr << "NaN detected in prediction! Reverting to last valid state." << std::endl;

    // Reverting to initial state (the state before the prediction)
    std::lock_guard<std::mutex> guard(_mutexEstimate);
    est.pos = _pos;
    est.vel = _vel;
    est.att = _att;
...
void VIOStateEstimator::UpdateWithMeasurement(Vec3d const measPos,
...
void VIOStateEstimator::UpdateWithVIO(Vec3d const measPos, Rotationd const measAtt, 
                                        Vec3d const measVel, Vec3d const measAngVel) {
  std::lock_guard<std::mutex> guard(_mutexEstimate);
  ...
  // Directly overwrite the state with measurements, assuming they are ground truth
  _pos = measPos;
  _vel = measVel;
  _att = measAtt;
  _angVel = measAngVel;

  // Optionally reset variances or set them to small values
  _variancePosition = Eigen::Matrix<double, 2, 2>::Identity() * 1e-4;
  _varianceAttitude = Eigen::Matrix<double, 2, 2>::Identity() * 1e-4;

  double const t0 = _estimateTimer.GetSeconds<double>(); // Estimator manual clock
  double const tEnd = _timer.GetSeconds<double>();  // Wall clock


  if (tEnd > t0){
    double dtInt = tEnd - t0;
    _estimateTimer.AdvanceMicroSeconds(uint64_t(0.5 + dtInt * 1e6));
  }
  
  // Update the timer to reflect that a new measurement has been incorporated
  _lastGoodMeasUpdate.Reset();
  
  // Clear expired messages to keep the prediction pipe clean
  _predictionPipe.ClearExpiredMessages(_estimateTimer.GetSeconds<double>());
}
...
//   // First set everything to measurements, assume ground truth
...
//   // Directly from measurements, variance is zero
//   _variancePosition = Eigen::Matrix<double, 2, 2>::Zero();
//   _varianceAttitude = Eigen::Matrix<double, 2, 2>::Zero();

//   _variancePosition(0, 0) = 1e-4;
//   _variancePosition(1, 1) = 1e-4;
//   _varianceAttitude(0, 0) = 1e-4;
//   _varianceAttitude(1, 1) = 1e-4;
