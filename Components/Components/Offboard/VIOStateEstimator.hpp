...
class VIOPredictionPipe {
  static constexpr double SMALL_TIME = 1e-6;  // [s]
 public:
  VIOPredictionPipe(BaseTimer* const timer, double delayTime)
      : _timer(timer),
...
/**! A state estimator that tracks a 6DOF object, using VIO outputs as measurements
...
class VIOStateEstimator {
 public:
  struct VIOEstimatedState {
    Vec3d pos, vel, angVel;
    Rotationd att;
  };

  VIOStateEstimator(BaseTimer* const masterTimer, unsigned const id,
  ...
  VIOEstimatedState GetPrediction(double const dt) const;
  ...
  void UpdateWithVIO(Vec3d const position, Rotationd const attitude, Vec3d const velocity, Vec3d const angVelocity);
 ...
 private:
  ...
  double _measNoiseStdDevVel;  // [m/s]
  double _measNoiseStdDevAngVel;  // [rad/s]
  ...
  VIOPredictionPipe<PredictionType> _predictionPipe;
