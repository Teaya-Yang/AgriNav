...
double const systemLatencyTime = 22e-3;  // latency in measurements & commands[s]

Vec3d returnPosition(1.0, 0, 1.5);
Vec3d recoverPosition(0.0, 0, 3.0);
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "demo_throw" + std::to_string(vehicleId));
  ...
  VehicleStateMachine_demoThrow veh;
  veh.Initialize(vehicleId, "quad", n, &timer, systemLatencyTime);

  veh.SetSafeCorners(Vec3d(-4, -4, -0.5), Vec3d(4, 4, 7), 1);

  double desiredYawAngle = 0 * M_PI / 180.0;

  cout << "Desired return position setpoint = <" << returnPosition.x << ","
       << returnPosition.y << "," << returnPosition.z << ">\n";

  veh.SetRecoverPosition(recoverPosition);
  veh.SetReturnPosition(returnPosition);
