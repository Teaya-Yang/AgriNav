...
int main(int argc, char **argv) {
  ...
  printf("Vehicle ID = %d (%s)\n", int(vehicleId), Onboard::QuadcopterConstants::GetNameStringFromID(vehicleId));
  ...
  QuadEvaluatorStateMachine veh;
  veh.Initialize(vehicleId, "rates vehicle", n, &timer, systemLatencyTime);

  double mass = Onboard::QuadcopterConstants(Onboard::QuadcopterConstants::GetVehicleTypeFromID(vehicleId)).mass;
  veh.SetMass(mass);

  Vec3d desiredPosition(0, 0, 2.0);
