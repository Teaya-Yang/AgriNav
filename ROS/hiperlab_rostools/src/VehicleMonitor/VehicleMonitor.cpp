    ...
    : _timeSinceLastRun(t)
{
  ...
  _subTel.reset(
      new ros::Subscriber(
          n.subscribe("telemetry" + std::to_string(_id), 10,
                      &VehicleMonitor::CallbackTelemetry, this))); // increase queue size to avoid dropping messages
  _subCmd.reset(
      new ros::Subscriber(
          n.subscribe("radio_command" + std::to_string(_id), 10,
