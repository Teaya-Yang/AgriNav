...
RappidsPerceptionAware::RappidsPerceptionAware(ros::NodeHandle &nh, ros::NodeHandle &private_nh)
  ...
  if (_is_sim){
    _subTracking = _nh.subscribe("/firefly/ground_truth/odometry", 1, &RappidsPerceptionAware::CallbackOdometry, this);
    _subTelemetry = _nh.subscribe("/firefly/imu", 1, &RappidsPerceptionAware::CallbackIMU, this);
    _subDepthImages = _it.subscribeCamera("/r200/camera/depth/image_raw", 1, &RappidsPerceptionAware::CallbackDepthImages, this, image_transport::TransportHints());
    _ref_pub = _nh.advertise<trajectory_msgs::MultiDOFJointTrajectory>("firefly/command/trajectory", 10);
  } else{
    if (_use_px4_odom){
      _subTracking = _nh.subscribe("/mavros/local_position/odom", 1, &RappidsPerceptionAware::CallbackOdometry, this);
      ROS_INFO("Use odometry from px4's odometry.");
    }else{
      _subTracking = _nh.subscribe("/ov_msckf/odomimu", 1, &RappidsPerceptionAware::CallbackOdometry, this);
      ROS_INFO("Use odometry from openvins.");
    }
    _subTelemetry = _nh.subscribe("/d455/imu", 1, &RappidsPerceptionAware::CallbackIMU, this);
    ...
    _subVehState = _nh.subscribe("/mavros/state", 1, &RappidsPerceptionAware::CallbackState, this);    
    _subRC = _nh.subscribe("/mavros/rc/in", 1, &RappidsPerceptionAware::CallbackRCIn, this);
    _ref_pub = _nh.advertise<mavros_msgs::PositionTarget>("mavros/setpoint_raw/local", 1);
    //Rotates vectors in the global frame (from openvins) to attitude frame
    _globalAtt = Rotation::FromEulerYPR(-90.0 * M_PI / 180.0, 0.0 * M_PI / 180.0, 0.0 * M_PI / 180.0);
  }
...
void RappidsPerceptionAware::CallbackRCIn(const mavros_msgs::RCIn::ConstPtr &msg){
    uint16_t landingSwitchValue = msg->channels[9];
    uint16_t const threshold = 1600;
    if (landingSwitchValue > threshold){
        _execute_last_traj = true;
        ROS_INFO("Landing triggered!");
    }
}

void RappidsPerceptionAware::CallbackState(const mavros_msgs::State::ConstPtr &msg) {
  _armed = msg->armed;
  _inOffboardMode = (msg->mode == "OFFBOARD");
}

void RappidsPerceptionAware::CallbackOdometry(
    const nav_msgs::Odometry &msg) {
  _odom_header = msg.header;
  if (_is_sim){
    //! position and orientation are expressed in Inertia frame,
    _est_pos = Vec3(msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z);
    _est_att = Rotation(msg.pose.pose.orientation.w, msg.pose.pose.orientation.x, msg.pose.pose.orientation.y, msg.pose.pose.orientation.z);
    //! linear velocity and angular velocity are expressed in Body frame
    //! rewritten in Inertia frame
    _est_vel = _est_att * Vec3(msg.twist.twist.linear.x, msg.twist.twist.linear.y, msg.twist.twist.linear.z);
  } else{
    if (_use_px4_odom){
      _odom_header.frame_id = "odom"; 
      //! position and orientation are expressed in Inertia frame,
      _est_pos = Vec3(msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z);
      _est_att = Rotation(msg.pose.pose.orientation.w, msg.pose.pose.orientation.x, msg.pose.pose.orientation.y, msg.pose.pose.orientation.z);
      //! linear velocity and angular velocity are expressed in Body frame
      ...
      //it's a bit confusing here, openvins gives data in the "global" frame, not the odom frame we want
      _odom_header.frame_id = "odom"; 
      _est_pos =  _globalAtt * Vec3(msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z);
      Rotation R_imu_to_global = Rotation(msg.pose.pose.orientation.w, msg.pose.pose.orientation.x, 
                                          msg.pose.pose.orientation.y, msg.pose.pose.orientation.z);
      _est_att =  _globalAtt * R_imu_to_global * _depthCamAtt.Inverse();
      //! linear velocity from openvins are expressed in the global frame
      _est_vel = _globalAtt * Vec3(msg.twist.twist.linear.x, msg.twist.twist.linear.y, msg.twist.twist.linear.z);
    }
  }
  //ROS_INFO("received odometry");
}

void RappidsPerceptionAware::CallbackIMU(const sensor_msgs::Imu &msg) {
  if (_is_sim){
    //! imu readings in Body frame, converted to odometry frame
    _est_acc = _est_att * Vec3(msg.linear_acceleration.x, msg.linear_acceleration.y, msg.linear_acceleration.z) - Vec3(0, 0, 9.81);
  }else{
    //! imu readings in the d455 imu frame (same as camera frame), converted to odometry frame
    _est_acc = _est_att * _depthCamAtt * Vec3(msg.linear_acceleration.x, msg.linear_acceleration.y, msg.linear_acceleration.z)
               - Vec3(0, 0, 9.81);
  }
}
...
void RappidsPerceptionAware::CallbackFeatures(const sensor_msgs::PointCloud &msg){
  ...
  if (_is_sim){
    for(const geometry_msgs::Point32& feature : msg.points){
      // active feature points in the odometry frame, from the feature publisher
    ...
    for(const geometry_msgs::Point32& feature : msg.points){
      ...
      Vec3 featurePointOdom = Vec3(feature.x, feature.y, feature.z);
      featurePointOdom = _globalAtt * featurePointOdom;
      _features_odom.push_back(featurePointOdom);
    }    
  }
...
void RappidsPerceptionAware::CallbackDepthImages(
  ...
  if(_flightStage == StageFlight){
    ...
    Vec3 vel_picture = _depthCamAtt.Inverse() * _est_att.Inverse() * _lastcmdVel;
    Vec3 acc_picture = _depthCamAtt.Inverse() * _est_att.Inverse() * _lastcmdAcc;
    Vec3 g_picture = _depthCamAtt.Inverse() * _est_att.Inverse() * Vec3(0, 0, -9.81);
    ...
    if (trajectory_found) {
      ...
      _trajAtt = _est_att * _depthCamAtt;
      _trajOffset = _est_pos;
...
void RappidsPerceptionAware::Run(const bool& shouldStart) {
  ...
  switch (_flightStage) {
    case StageWaitForStart:
      ...
      //Frequency must be faster than 2Hz.
      //Otherwise the mode switch will be rejected.
      //This is ugly but this is how px4 works...
      if (!_is_sim){
        current_waypoint = WaypointWithTime(0, Vec3(0,0,0), Vec3(0,0,0), 
                                      Vec3(0,0,0), 0, 0);
        publishReferenceCmd(current_waypoint);
      }
      if (shouldStart){
    ...
    case StageTakeoff:
      if (stageChange){
        ROS_INFO("Taking off");
        Vec3 desired_direction_odom = Vec3(_goal_posx,_goal_posy,_goal_posz) - _est_pos;    
        ...
        if (_planner_started) {
          if(_goal_reached){
            ...
            if ((ros::Time::now() - _stateStartTime).toSec() > hoverTime){
              ...
              _pos_before_landing = _est_pos;
            }
            current_waypoint = WaypointWithTime(0, _est_pos, Vec3(0,0,0), 
                                          Vec3(0,0,0), _desired_yaw, 0);
          }else{
          ...
          current_waypoint = WaypointWithTime(0, _est_pos, Vec3(0,0,0), Vec3(0,0,0), _desired_yaw, 0);
    ...
    case StageLanding:
      ...
      {
        double const landing_speed = 0.4; //[m/s]
        double cmd_height = _pos_before_landing.z
            - landing_speed * (ros::Time::now() - _stateStartTime).toSec();
        current_waypoint = WaypointWithTime(0, Vec3(_pos_before_landing.x, _pos_before_landing.y, cmd_height), Vec3(0,0,0), Vec3(0,0,0), _desired_yaw, 0);
        publishReferenceCmd(current_waypoint);
