...
#include <cerrno>
#include <cmath>
#include "../include/rappids/rapid_trajectory_generator.hpp"
#include "../include/math/quartic.h"
...
RapidTrajectoryGenerator::StateFeasibilityResult RapidTrajectoryGenerator::CheckVelocityFeasibility(double vmax) {
  ...
  for (int dim = 0; dim < 3; dim++) {
    ...
    if (fabs(c[0]) > 1e-6) {
      rootCount = Quartic::solveP3(c[1] / c[0], c[2] / c[0], c[3] / c[0], roots);
    } else {
    ...
    for (unsigned i = 0; i < (rootCount + 2); i++) {
      ...
      Vec3 velAtRoot = GetVelocity(roots[i]);
      if (std::abs(velAtRoot.x) >= vmax || std::abs(velAtRoot.y) >= vmax || std::abs(velAtRoot.z) >= vmax) {
